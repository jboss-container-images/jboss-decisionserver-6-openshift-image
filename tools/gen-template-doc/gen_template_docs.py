...
LINKS =  {
          "jboss-decisionserver64-openshift:1.4": "../../decisionserver-openshift{outfilesuffix}[`jboss-decisionserver-6/decisionserver64-openshift`]",
}
...
def generate_template(path):
    ...
    with open(path) as data_file:
        ...
        outfile = re.sub('docs/secrets', 'docs', outfile)
        outfile = re.sub('docs/templates', 'docs', outfile)
...
def createTemplate(data, path):
    templater = Template()
    templater.template = open('./tools/gen-template-doc/template.adoc.in').read()
...
fullname = {
    "decisionserver": "Red Hat JBoss BRMS realtime decision server",
}

def generate_readme():
    """Generates a README page for the template documentation."""
    with open('templates/docs/README.adoc','w') as fh:
        fh.write(autogen_warning)
        # page header
        fh.write(open('./tools/gen-template-doc/README.adoc.in').read())
        ...
        for directory in sorted(template_dirs):
            ...
            for template in [ os.path.splitext(x)[0] for x in sorted(os.listdir(directory)) ]:
                if "image-stream" not in template:
                    if template != "docs":
                        fh.write("* link:./%s/docs/%s.adoc[%s]\n" % (directory, template, template))

        # release notes
        fh.write(open('./tools/gen-template-doc/release-notes.adoc.in').read())
